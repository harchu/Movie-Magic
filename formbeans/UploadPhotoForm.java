...
public class UploadPhotoForm extends FormBean {
	...
	public List<String> getValidationErrors() {
		...
		if (lastDot == -1) {
			errors.add("No extension on file name.  Must be one of: "+Photo.EXTENSIONS);
			return errors;
		}
		
		String extension = file.getFileName().substring(lastDot);
		if (!Photo.EXTENSIONS.contains(extension)) {
			errors.add("Invalid extension on file name.  Must be one of: "+Photo.EXTENSIONS);
		}
